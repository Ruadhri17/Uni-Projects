...
class Sequence{
    ...
    public:
        ...
        Key getHeadKey(); // function returning Key of Sequence's head
        Info getHeadInfo(); // function returning Info of Sequence's head
};
template <typename Key, typename Info>
...
{
    Sequence<Key, Info> temp1 = s1;
    Sequence<Key, Info> temp2 = s2;
    ...
    for(int k = 0; k < count; k++)
        {
                    for(int i = 0; i < len1; i++)
                        { 
                            if(temp1.isEmpty())
                                break;
                            NewSequence.pushBack(temp1.getHeadKey(), temp1.getHeadInfo());
                            temp1.popFront();
                        }
                    for(int j = 0; j < len2; j++)
                        {
                            if(temp2.isEmpty())
                                break;
                            NewSequence.pushBack(temp2.getHeadKey(), temp2.getHeadInfo());
                            temp2.popFront();
                        }
...
template <typename Key, typename Info>
Key Sequence<Key, Info>::getHeadKey()
{
    return head->key_of_node;
}
template <typename Key, typename Info>
Info Sequence<Key, Info>::getHeadInfo() 
{
    return head->info_of_node;
}
#endif